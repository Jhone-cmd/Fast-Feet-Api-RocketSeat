...
type EmployeeResponse = Either<EmployeeAlreadyExists, { employee: Employee }>;
export class RegisterEmployeeUseCase {
	...
	async execute({
		...
		if (employeeWithSameEmail) {
			return left(new EmployeeAlreadyExists(email));
		}
		...
		if (employeeWithSameCPF) {
			return left(new EmployeeAlreadyExists(cpfFormatted.value));
		}
